...
* [Using](#using)
  * [JAR file contents](#jar-file-contents)
...
The Atex OneCMS Code Library is distributed as a Maven JAR artifact which is depended upon like this:
...
The `FileService` object is a low-level service that is used to transfer files to and from the file service in the Content REST API. `atex.onecms.service.FileService` is a function returning an instance of the `FileService` API:
...
atex.onecms.service.FileService = function(fileServiceUrl, serviceSettings) {
  return {
     ...
     * event handler on the object returned from this method. Example:
     *
     ...
     * Please see the Content API File service documentation for more information.
     *
     * Rejects the returned promise with an atex.onecms.error.Error object if the
     * operation fails.
     *
     * For progress updates on the blob object upload, please attach an 'onProgress'
     * event handler on the object returned from this service. Example:
     *
...
transferInfo.putFile(transferInfo).then(function(response) {
  // Do something with the response...
}, function(error) {
  // The operation failed. Inspect the 'error' object (type atex.onecms.error.Error)
  // for more information.
...
['FileService', function(FileService) {
  ...
  FileService.putFile(transferInfo).then(function(response) {
    // Do something with the response...
  }, function(error) {
    // The operation failed. Inspect the 'error' object (type atex.onecms.error.Error)
    // for more information.
